let titleText = "LOOP";
let descriptionText = "In a world on the brink of extinction, ancient witch doctors merge themselves with technology and earthly elements in a desperate attempt to preserve life. They craft a simulated reality, becoming hidden architects of this new existence.";
...
async function loadCharactersData() {
    try {
        const response = await fetch('charactersData.json');
        if (!response.ok) {
...
function initializeGallery() {
    gallery = document.querySelector('.gallery'); // inicializa a variável global ou local
    if (!gallery) {
    ...
    for (let i = 0; i < charactersData.length; i++) {
        ...
        if (file && index !== undefined && file.fileType !== undefined) {
            img.src = `Collection/${index}.${file.fileType}`;
        } else {
            console.error('Dados de arquivo ausentes ou incompletos:', file);
        }
        const figcaption = document.createElement('figcaption');

        attributesImg = document.createElement('img');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Conteiner para Título e Descrição
    ...
    document.body.appendChild(titleDescriptionContainer);
...
// Conteiner para Footer (futuro)
const footerContainer = document.createElement('div');
footerContainer.className = 'footer-container';
// ... (adicionar footer aqui no futuro)
document.body.appendChild(footerContainer);

// Conteiner para Menu no topo (futuro)
const topMenuContainer = document.createElement('div');
topMenuContainer.className = 'top-menu-container';
// ... (adicionar menu no topo aqui no futuro)
document.body.appendChild(topMenuContainer);

// Conteiner para Filtro de busca (futuro)
const searchFilterContainer = document.createElement('div');
searchFilterContainer.className = 'search-filter-container';
// ... (adicionar filtro de busca aqui no futuro)
